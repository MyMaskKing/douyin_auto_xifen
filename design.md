...
5. **粉丝关注管理模块 (core/follow_fans_manager.py)**：负责处理视频评论和群组用户的关注操作。
6. **数据库模块 (utils/db.py)**：负责数据的存储和检索，包括用户信息、关注记录等。
7. **配置模块 (utils/config.py)**：负责读取和验证配置信息。
8. **日志模块 (core/logger.py)**：负责记录系统运行日志。
...
features:
  ...
  check_fans: true        # 是否启用检查粉丝列表功能
  follow_back: true       # 是否启用回关功能
  follow_video_fans: false  # 是否启用视频评论关注功能
  follow_group_fans: false  # 是否启用群组用户关注功能
```
